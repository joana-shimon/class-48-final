...
tesseract=createSprite(width*2,200,50,50)
...
for(d=0;d<blackArr.length;d++){
    if(d!==0 && ironMan.isTouching(blackArr[d])){
        ironMan.x=blackholePos[d-1][0]-200
...
if(ironMan.isTouching(tesseract)){
    question()
    ironMan.x-=100
}
...
bulletGroup.add(bullet)
 ...
 function question(){
     swal({
         title:`Question to collect the Space Stone `,
         text:"what is your name",
        // confirmButtonText:"OK"
         
     })
     input=createInput("type your answer")
     input.position(width-500,200)
     if(input.value()==="Iron Man"){
         spaceStone.x=ironMan.x+500
         spaceStone.visible=true
